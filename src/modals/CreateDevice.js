...
import { Context } from "..";
import { Disclosure, Menu, Dialog, RadioGroup, Transition } from '@headlessui/react';
...
const CreateDevice = ({show, onHide}) => {
    ...
    return (
                    ...
                    <div className="flex min-h-full items-stretch justify-center text-center md:items-center md:px-2 lg:px-4">
                        ...
                        <Dialog.Panel className="flex w-full transform text-left text-base transition md:my-8 md:max-w-2xl md:px-4 lg:max-w-4xl">
                            ...
                            <div className=" grid w-full grid-cols-1 items-start gap-x-6 gap-y-8 sm:grid-cols-12 lg:gap-x-8">
                                ...
                                <h2 className="text-2xl font-bold text-gray-900 sm:pr-12">Создать новый продукт</h2>
                                ...
                                <section aria-labelledby="options-heading" className="mt-10">
                                    <div>
                                        ...
                                        <div>
                                            ...
                                            {typeField && typeField.name || "Вибрать категорию"}
      ...
      >
                                            ...
                                            <div className="py-1">
                                                {device.types.map((type) => <Menu.Item>
                                                    <item
                                                    onClick={() => setType(type)}
                                                    key={type}
                                                    className={classNames(
                                                        typeField && typeField.name === type.name ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                                        ...
                                        <div>
                                            <Menu.Button className="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                                            {brandField && brandField.name || "Вибрать тип"}
                                            <ChevronDownIcon className="-mr-1 h-5 w-5 text-gray-400" aria-hidden="true" />
                                            </Menu.Button>
                                        </div>
                                        <Transition
      ...
      >                                     
                                            <Menu.Items className="absolute left-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                            <div className="py-1">
                                                {device.brands.map((brand) => <Menu.Item>
                                                    <item   
                                                    onClick={() => setBrand(brand)}
                                                    key={brand}
                                                    className={classNames(
                                                        brandField && brandField.name === brand.name ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                                                        'block px-4 py-2 text-sm hover:bg-gray-100 hover:text-gray-900'
                                                    )}
                                                    >
                                                    {brand.name} {brandField == brand.name}
                                                    </item>
                                                </Menu.Item>)}
                                            </div>
                                            </Menu.Items>
                                        ...
                                        {info.map((item, index) => (
                                            <div key={item.number} className="flex items-center my-2">
                                            <input
                                                type="text"
                                                placeholder="Name"
                                                value={item.title}
